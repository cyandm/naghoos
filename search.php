        ...
        <div id="searchPostType" class="container">
            ...
            <form id="search-form" method="get" action="<?php echo esc_url(home_url('/')); ?>" class="flex justify-between items-center max-md:flex-col max-md:gap-6 max-md:px-4 max-md:py-5 max-md:border border-cynBlack/10 rounded-3xl">
                ...
                <div class="relative max-md:w-full">
                    ...
                    <div class="absolute inset-0 top-1/2 -translate-y-1/2 flex items-center ps-3.5 pointer-events-none text-cynRed size-9">
                    ...
                    <input type="text"
                        ...
                        class="text-cynBlack text-base font-medium rounded-2xl border border-cynRed focus:outline-none focus:ring-cynRed focus:border-cynRed block w-full ps-10 p-3 pt-3.5 transition-all duration-200 md:min-w-64"
                        placeholder="<?php _e('دنبال چی میگردی؟', 'naghoos'); ?>">
                </div>
                ...
                <div class="flex justify-between items-center gap-2 md:gap-6 text-base font-medium max-sm:w-full">
                    ...
                    <div class="text-base font-medium text-cynBlack">
                        <?php _e('جستجو در', 'naghoos'); ?> :
                    </div>

                    <div class="flex gap-4">
                        <div class="flex justify-center items-center gap-1">
                            <input class="peer custom filter w-6 h-6 accent-cynRed"
                                value="all"
                                type="radio"
                                name="search-type"
                                id="search-all"
                                <?php echo $search_type === 'all' ? 'checked' : '' ?>>
                            <label for="search-all" class="text-sm font-medium text-cynBlack/50 peer-checked:text-cynBlack cursor-pointer"><?php _e('همه', 'naghoos'); ?></label>
                        </div>
                        ...
                        <div class="flex justify-center items-center gap-1">
                            ...
                            <label for="search-product" class="text-sm font-medium text-cynBlack/50 peer-checked:text-cynBlack cursor-pointer"><?php _e('فروشگاه', 'naghoos'); ?></label>
                        </div>
                        ...
                        <div class="flex justify-center items-center gap-1">
                            ...
                            <label for="search-blog" class="text-sm font-medium text-cynBlack/50 peer-checked:text-cynBlack cursor-pointer"><?php _e('بلاگ', 'naghoos'); ?></label>
                        </div>
                ...
                <?php if (! empty($_GET['s'])) : ?>
                    <div class="flex items-center gap-1 text-base font-light text-cynBlack/60">
                        ...
                        <span><?php _e('نتیجه', 'naghoos'); ?></span>
                    </div>
                <?php endif ?>
            </form>
    ...
    <section class="container">
        <?php if (! empty($_GET['s'])) : ?>
            ...
            <?php if (have_posts()) : ?>
                ...
                <div id="searchPostsWrapper" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    <?php while (have_posts()) :
                        the_post();
                        $is_product = (get_post_type() === 'product');
                    ?>
                        <div class="<?php echo $is_product ? '' : 'sm:col-span-1'; ?>">
                            <?php if ($is_product) : ?>
