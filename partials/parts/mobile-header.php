...
<section class="mobile-menu-scroll h-screen bg-white p-5 w-0 fixed inset-0 z-50 opacity-0 pointer-events-none overflow-y-auto data-[active='true']:w-full data-[active='true']:opacity-100 data-[active='true']:pointer-events-auto duration-500" modal data-modal-name="menu-modal" data-active="false">
	...
	<div class="flex justify-between items-center">
		...
		<div class="size-8 flex justify-center items-center text-cynRed" modal-closer data-modal-name="menu-modal">
			<?php Icon::print('Arrow,-Forward'); ?>
		</div>

		<?php if ($logo_mobile_menu) : ?>
			<div><img src="<?php echo $logo_mobile_menu ?>" alt="logo" class="object-contain"></div>
		<?php else : ?>
			<div class="size-12"><?php the_custom_logo() ?></div>
		<?php endif; ?>
	...
	<div class="mt-2.5">
		...
		<?php wp_nav_menu([
			'menu_id' => 'mobile-menu',
			'menu_class' => 'gap-0.5 [&>li]:border-t [&>li]:border-[#F7EAEA] [&>li]:first:border-t-0 flex-col text-cynBlack [&_li_a]:flex [&_li_a]:py-3 [&_li_a]:w-full text-base font-medium [&_li_ul]:px-3',
			'depth' => '3',
