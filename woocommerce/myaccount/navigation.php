 ...
 * that includes custom menu items (notifications, support).
 *
...
global $wp;
$current_endpoint = '';
$notifications_unread_count = is_user_logged_in()
	? Notifications::getUnreadCountForUser(get_current_user_id())
	: 0;
...
$allowed_endpoints = array('orders', 'view-order', 'downloads', 'notifications', /* 'wishlist', */ 'support');
...
<div class="lg:hidden">
	<details class="group relative">
		<summary class="list-none cursor-pointer">
			<div
				class="w-full px-6 py-2.5 rounded-2xl border border-cynRed bg-white text-black flex flex-row-reverse items-center justify-between text-sm">
				<span class="flex items-center gap-2 shrink-0">
					...
					<?php if ($notifications_unread_count > 0 && $current_endpoint !== 'notifications'): ?>
						<span class="text-[10px] min-w-5 h-5 px-1.5 rounded-full bg-red-500 text-white inline-flex items-center justify-center font-medium leading-none">
							<?php echo esc_html((string) $notifications_unread_count); ?>
						</span>
					<?php endif; ?>
				</span>
				<span class="flex items-center gap-2 shrink-0">
		...
		<div class="myaccount-mobile-nav-panel mt-2 bg-white border border-gray-200 rounded-2xl p-2 space-y-1">
			<?php foreach ($menu_items as $endpoint => $item): ?>
				...
				$mobile_state_classes = $is_disabled
					...
					: ($is_active ? 'bg-cynRed text-cynWhite' : 'text-gray-900 hover:bg-slate-100');
				...
				<?php if ($is_disabled): ?>
					<span class="<?php echo esc_attr($mobile_item_classes); ?>">
						<span class="flex items-center gap-2">
							...
							<span class="text-[10px] bg-cynRed text-cynWhite px-2 py-0.5 rounded-full">بزودی</span>
						</span>
					</span>
				<?php else: ?>
					<a href="<?php echo esc_url($item_url); ?>" class="<?php echo esc_attr($mobile_item_classes); ?>">
						<span class="flex items-center gap-2">
							<span class="<?php echo esc_attr($is_active ? 'text-cynWhite' : 'text-cynRed'); ?>">
								<?php echo $item['icon']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
								?>
							</span>
							<span><?php echo esc_html($item['label']); ?></span>
						</span>
						<?php if ($endpoint === 'notifications' && $notifications_unread_count > 0 && ! $is_active): ?>
							<span class="text-[10px] min-w-5 h-5 px-1.5 rounded-full bg-red-500 text-white inline-flex items-center justify-center font-medium leading-none">
								<?php echo esc_html((string) $notifications_unread_count); ?>
							</span>
						<?php endif; ?>
			...
			<button type="button" modal-opener data-modal-name="logout-confirm-modal"
				...
				<span class="flex items-center gap-2">
					<svg class="w-5 h-5 text-cynRed" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="hidden lg:block sticky top-6 self-start bg-white rounded-2xl border-gray-200 border-1 p-4 space-y-2">
	<?php foreach ($menu_items as $endpoint => $item): ?>
		...
		$active_classes = $is_active ? 'bg-cynRed text-cynWhite' : '';
		...
		<?php if ($is_disabled): ?>
			<span class="<?php echo esc_attr($item_class); ?>">
				<div class="flex items-center gap-2">
					...
					<span><?php echo esc_html($item['label']); ?></span>
					<span class="text-xs bg-cynRed text-cynWhite px-2 py-0.5 rounded-full">بزودی</span>
				</div>
				<div class="w-5"></div>
			</span>
		<?php else: ?>
			<a href="<?php echo esc_url($url); ?>" class="<?php echo esc_attr($item_class); ?>">
				<div class="flex items-center gap-2">
					<div class="<?php echo esc_attr($is_active ? 'text-cynWhite' : 'text-cynRed'); ?>">
						<?php echo $item['icon']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
						?>
					</div>
					<span><?php echo esc_html($item['label']); ?></span>
				...
				<div class="flex items-center gap-2">
					<?php if ($endpoint === 'notifications' && $notifications_unread_count > 0 && ! $is_active): ?>
						<span class="text-[11px] min-w-5 h-5 px-1.5 rounded-full bg-red-500 text-white inline-flex items-center justify-center font-medium leading-none">
							<?php echo esc_html((string) $notifications_unread_count); ?>
						</span>
					<?php elseif ($is_active): ?>
						<svg class="w-5 h-5 text-cynWhite" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<div
		...
		<div class="w-full flex items-center justify-center gap-3">
			<a href="<?php echo esc_url($logout_url); ?>"
				class="w-[120px] h-10 rounded-full bg-cynRed border border-cynRed text-cynWhite text-sm font-medium flex items-center justify-center hover:bg-cynRed/90 transition-colors duration-200">
				تایید
			</a>
			<button type="button" modal-closer data-modal-name="logout-confirm-modal"
				class="w-[120px] h-10 rounded-full border border-cynRed text-cynBlack text-sm font-medium flex items-center justify-center hover:bg-cynRed/10 transition-colors duration-200">
				انصراف
			</button>
		</div>
	</div>
