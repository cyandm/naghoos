...
$desktop_archive_banner = null;
$mobile_archive_banner  = null;
$archive_banner_link    = null;

if (is_product_category() && isset($term->term_id)) {
	$term_id = $term->term_id;
	$desktop_archive_banner = get_field('desktop_archive_banner', 'product_cat_' . $term_id);
	$mobile_archive_banner  = get_field('mobile_archive_banner', 'product_cat_' . $term_id);
	$archive_banner_link    = get_field('archive_banner_link', 'product_cat_' . $term_id);
}
...
<main class="archive-product">
	...
	<section class="grid grid-cols-1 lg:grid-cols-13 gap-6 container">
		...
		<div class="lg:col-span-9 xl:col-span-10 order-1 lg:order-2">
			...
			<?php if (!empty($desktop_archive_banner) && !empty($mobile_archive_banner)): ?>
				<a href="<?php echo !empty($archive_banner_link) ? esc_url($archive_banner_link) : '#'; ?>" class="archive-banner w-full mb-4 flex">
					<?php
					echo wp_get_attachment_image($desktop_archive_banner, 'full', false, ['class' => 'w-full object-cover object-top max-md:hidden']);
					echo wp_get_attachment_image($mobile_archive_banner, 'full', false, ['class' => 'w-full object-cover object-top md:hidden']);
					?>
				</a>
			<?php endif; ?>
			...
			?>
				...
				<!-- Products Grid -->
				<div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-3 mt-5 [&_.product-card]:h-full">
					<?php
