...
export function ProductVariation() {
  ...
  function setSelectAndTrigger(attrName, value) {
    // اصلاح: اگر خودِ attrName از قبل شامل 'attribute_' هست، آن را تکرار نکن
    const selectorName = attrName.startsWith("attribute_")
      ? attrName
      : `attribute_${attrName}`;
    const $sel = $form.find(`select[name="${selectorName}"]`);

    console.log("تلاش برای پیدا کردن سلکتور با نام:", selectorName);
    ...
    if ($sel.length && value) {
      ...
      console.warn("سلکتور پیدا نشد یا مقدار خالی است!");
    }
  }
